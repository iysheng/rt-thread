...
void gd32_usart_gpio_init(struct gd32_usart_bus *bus)
{
    ...
    gpio_init(bus->config->tx_port, GPIO_MODE_AF_PP, GPIO_OSPEED_10MHZ, bus->config->tx_pin);
    gpio_init(bus->config->rx_port, GPIO_MODE_IN_FLOATING, GPIO_OSPEED_10MHZ, bus->config->rx_pin);
