...
static void can_thread_entry(void *parameter)
{
    ...
    {
        rt_kprintf("find %s success", can_name);
    }
 ...
 * pinmux init
 *
 * PA109 -> UART9 TXD9 101b
 * PA110 -> UART9 RXD9 101b
 * */
void app_init(void) 
{
    __attribute__((unused)) rt_thread_t tid;
#ifdef RT_USING_HEAP
    tid = rt_thread_create("can", can_thread_entry, RT_NULL,
