...
static void gd32_pin_write(rt_device_t dev, rt_base_t pin, rt_base_t value)
{
    ...
    if (PIN_PORT(pin) < PIN_GDPORT_MAX)
    {
        ...
        gpio_bit_write(gpio_port, gpio_pin, (BitState)value);
    }
}
...
static void gd32_pin_mode(rt_device_t dev, rt_base_t pin, rt_base_t mode)
{
    ...
    switch (mode)
    {
        case PIN_MODE_OUTPUT:
            ...
            pin_pull_down = GPIO_PUPD_PULLUP;
            break;
        case PIN_MODE_INPUT:
            pinmode = GPIO_MODE_INPUT;
            pin_pull_down = GPIO_PUPD_NONE;
            break;
        ...
        case PIN_MODE_INPUT_PULLDOWN:
            pinmode = GPIO_MODE_INPUT;
            pin_pull_down = GPIO_PUPD_PULLDOWN;
            break;
    ...
    gpio_mode_set(PIN_GDPORT(pin), pinmode, pin_pull_down, PIN_GDPIN(pin));
}
                                     ...
                                     rt_uint32_t enabled)
{
    ...
    if (enabled == PIN_IRQ_ENABLE)
    {
        ...
        case PIN_IRQ_MODE_RISING:
            pin_pull_down = GPIO_PUPD_PULLDOWN;
            exti_init(irqindex, EXTI_INTERRUPT, EXTI_TRIG_RISING);
            break;
        case PIN_IRQ_MODE_FALLING:
            pin_pull_down = GPIO_PUPD_PULLUP;
            exti_init(irqindex, EXTI_INTERRUPT, EXTI_TRIG_FALLING);
            break;
        case PIN_IRQ_MODE_RISING_FALLING:
            exti_init(irqindex, EXTI_INTERRUPT, EXTI_TRIG_BOTH);
            break;
        default:
            break;
        }
